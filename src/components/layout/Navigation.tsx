  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'bg-surface-950/80 backdrop-blur-md border-b border-surface-800/50 shadow-lg'
          : 'bg-transparent'
      )}
    >
      <nav className="container-custom" aria-label="Main navigation">
        <div className="flex h-16 items-center justify-between">
          <a
            href="#hero"
            className="font-display text-display-sm font-bold text-foreground hover:opacity-80 transition-opacity"
          ...
          <div className="hidden md:flex md:items-center md:gap-8">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                className={cn(
                  'relative text-body-sm font-medium transition-colors hover:text-brand-500',
                  activeSection === link.href.slice(1) ? 'text-brand-500' : 'text-muted-foreground'
                )}
              >
                {link.label}
                {activeSection === link.href.slice(1) && (
                  <span className="absolute bottom-[-4px] left-0 right-0 h-[2px] bg-brand-500 rounded-full animate-scale-in" />
                )}
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              className="md:hidden p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-surface-800 transition-colors"
          ...
          <div className="flex flex-col gap-2 pb-6">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                className={cn(
                  'px-4 py-3 rounded-xl text-body font-medium transition-colors',
                  activeSection === link.href.slice(1)
            ...
            <Button variant="default" className="mx-4 mt-2" asChild>
              <a href="#contact">Get in Touch</a>
            </Button>
          </div>
        </div>
